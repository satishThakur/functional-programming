package com.satish.fp.tokenauth
import cats.MonadThrow
import cats.syntax.all.*
import pdi.jwt.{Jwt, JwtAlgorithm, JwtClaim}

import java.security.KeyFactory
import scala.util.{Failure, Success}

trait JwtService[F[_]]:
  def encodeAsymetric(): F[String]
  def decodeAsymetric(token: String): F[String]

object JwtService:
  val privateKey = "-----BEGIN PRIVATE KEY-----\\nMFtIeOWAFO63+e+u8VifENDd\n-----END PRIVATE KEY-----"
  val publicKey = "-----BEGIN PUBLIC KEY-----\\nXwIDAQAB\n-----END PUBLIC KEY-----"
  class JwtServiceImpl[F[_] : MonadThrow]() extends JwtService[F]:
    override def encodeAsymetric(): F[String] =
      Jwt.encode(JwtClaim("abcdef"), privateKey, JwtAlgorithm.RS256).pure

    override def decodeAsymetric(token: String): F[String] =
      Jwt.decode(token, publicKey, Seq(JwtAlgorithm.RS256)) match
        case Success(value) => value.content.pure
        case Failure(exception) => MonadThrow[F].raiseError(exception)