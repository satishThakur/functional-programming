...
object TokenApp extends IOApp.Simple:
  override def run: IO[Unit] =
    val claim = Claim.makeClaim[IO,UserClaim]
    val token = Token.makeSymToken[IO,UserClaim](claim, SecretKey("superSecret"))
    ...
    val asymToken = Token.makeAsymToken[IO, UserClaim](claim, AsymmetricKeyPair(privateKey, publicKey))
    println(user)
    println(anotherUser)
