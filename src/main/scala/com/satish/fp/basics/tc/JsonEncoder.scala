...
object JsonEncoder:
  ...
  inline def deriveCase[A,T <: Tuple](pos : Int, ord: Int, a: A) : String = inline erasedValue[T] match
    ...
    case _: (t *: ts) => {
      if pos == ord then
        summonFrom{
          case p: Mirror.ProductOf[`t`] => deriveProduct[t](p).encode(a.asInstanceOf[t])
        }
      else deriveCase[A, ts](pos + 1, ord, a)
    }

  inline def deriveSum[A](s: Mirror.SumOf[A]): JsonEncoder[A] =
    new JsonEncoder[A]:
      extension(a: A)
        def encode: String =
          val ord = s.ordinal(a)
  ...
  inline def deriveProduct[A](p: Mirror.ProductOf[A]): JsonEncoder[A] =
    new JsonEncoder[A]:
      extension(a: A)
        def encode: String =
